...
public class CommandManager {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if ((cmd.getName().equalsIgnoreCase("mydog")) || (cmd.getName().equalsIgnoreCase("md")) || (cmd.getName().equalsIgnoreCase("dog"))
			...
			case 1:
				...
				case "reload":
					...
					sender.sendMessage(ChatColor.YELLOW + this.plugin.getDescription().getFullName() + ":" + ChatColor.AQUA
							+ " Reloaded the configurations.");
					return true;
				...
				case "dead":
					if (player == null)
						return false;
					if (!plugin.allowRevival || ((!player.isOp()) && (!MyDog.getPermissionsManager()
			...
			case 2:
				...
				case "kill": {
					...
					try {
						...
						sender.sendMessage(
								ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.RED
										+ "Please enter a valid ID! Check /mydog dogs");
						return true;
					}
					...
					if (args[1].equalsIgnoreCase("all")) {
						...
						try {
							...
							sender.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET
									+ ChatColor.RED + "Please enter a valid ID! Check /mydog dogs");
							return true;
						}
					}
	...
	private boolean commandList(CommandSender sender) {
		...
		if (sender instanceof Player) {
			...
			if ((sender.isOp()) || (MyDog.getPermissionsManager().hasPermission(player, "mydog.comehere"))) {
				sender.sendMessage(
						ChatColor.AQUA + "/mydog comehere <id>" + ChatColor.WHITE + " - Forces your Dog to teleport to your location");
			}
			if ((sender.isOp()) || (MyDog.getPermissionsManager().hasPermission(player, "mydog.sit"))) {
				sender.sendMessage(ChatColor.AQUA + "/mydog sit <id | all>" + ChatColor.WHITE
						+ " - Tells your dog(s) to sit and keep their position(s)");
				sender.sendMessage(
						ChatColor.AQUA + "/mydog stand <id | all>" + ChatColor.WHITE + " - Tells your dog(s) to stand up and roam free");
			}
			if ((sender.isOp()) || (MyDog.getPermissionsManager().hasPermission(player, "mydog.togglemode"))) {
				sender.sendMessage(
						ChatColor.AQUA + "/mydog attack <id | all>" + ChatColor.WHITE + " - Tells your dog(s) to attack any mobs nearby");
			...
			if ((sender.isOp()) || (MyDog.getPermissionsManager().hasPermission(player, "mydog.trade"))) {
				sender.sendMessage(ChatColor.AQUA + "/mydog trade <id> <player> [price]" + ChatColor.WHITE
			...
			if ((sender.isOp()) || (MyDog.getPermissionsManager().hasPermission(player, "mydog.stats"))) {
				sender.sendMessage(
						ChatColor.AQUA + "/mydog info <id>" + ChatColor.WHITE + " - Gets stats and other info about a Dog you own");
			}
			...
			if ((sender.isOp()) || (MyDog.getPermissionsManager().hasPermission(player, "mydog.setid"))) {
				sender.sendMessage(
						ChatColor.AQUA + "/mydog setid <id> <newid>" + ChatColor.WHITE + " - Assigns a custom ID to a Dog you own");
			}
	...
	private boolean commandDogList(CommandSender sender) {
		...
		for (Map.Entry<Integer, Dog> entry : dogsSorted.entrySet()) {
			...
			sender.sendMessage(
					ChatColor.AQUA + "#" + entry.getValue().getIdentifier() + ChatColor.WHITE + " - " + ChatColor.AQUA + entry.getValue()
							.getDogName() + healthString);
		}
		return true;
	}
	...
	private boolean commandDogDead(CommandSender sender) {
		...
		for (Map.Entry<Integer, Dog> entry : dogsSorted.entrySet()) {
			sender.sendMessage(
					ChatColor.AQUA + "#" + entry.getValue().getIdentifier() + ChatColor.WHITE + " - " + ChatColor.AQUA + entry.getValue()
							.getDogName() + ChatColor.WHITE + " LVL " + entry.getValue().getLevel() + " " + ChatColor.GREEN + " $"
							+ entry.getValue().getRevivalPrice());
		}
		return true;
	}
	...
	private boolean commandDogRename(CommandSender sender, String[] args) {
		...
		try {
			...
			sender.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.RED
					+ "Please enter a valid ID! Check /mydog dogs");
			return false;
		}
		...
		if (dog == null) {
			sender.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.RED
					+ "Could not find a Dog with that ID! Check /mydog dogs");
			return false;
		}

		if ((name.length() == 0) || name.length() > 16) {
			sender.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.RED
					+ "Please choose a name between 1 and 16 characters for your Dog!");
			return false;
		}

		if (!dog.setDogName(name.toString())) {
			sender.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.RED
					+ "An error occured! Could not set Dog name!");
			return false;
		}

		sender.sendMessage(ChatColor.AQUA + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.AQUA
				+ "From now on, I will call you " + dog.getDogColor() + dog.getDogName() + ChatColor.RESET + ChatColor.AQUA + "!");
	...
	private boolean commandDogSetId(CommandSender sender, String[] args) {
		...
		try {
			...
			sender.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.RED
					+ "Invalid ID! Type /mydog dogs to find the ID.");
			return false;
		}
		...
		if (new_id.isEmpty() || new_id.length() > 10) {
			sender.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.RED
					+ "Please choose an identifier between 1 and 10 figures for your Dog!");
			return false;
		}
		...
		try {
			...
			sender.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.RED
					+ "Your Dog's identifier can only consist of numbers!");
			return false;
		}

		if (!MyDog.getDogManager().setNewId(dog, id)) {
			sender.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.RED
					+ "An error occurred! Could not set new Dog ID!");
			return false;
		}

		sender.sendMessage(ChatColor.AQUA + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.AQUA
				+ "New Dog ID successfully set!");
	...
	private boolean commandDogPutdown(CommandSender sender, int dogIdentifier) {
		...
		if (wolf == null) {
			sender.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.RED
					+ "Aww bonkers! Seems like your Dog cannot be found... Is it loaded?");
			return false;
		}
	...
	private boolean commandDogFree(CommandSender sender, int dogIdentifier) {
		...
		sender.sendMessage(ChatColor.AQUA + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.AQUA
				+ "It was great having you here, " + dog.getDogColor() + dog.getDogName() + ChatColor.RESET + ChatColor.AQUA + "...");
	...
	private boolean commandDogDefendAttack(CommandSender sender, int dogIdentifier, boolean toAttack) {
		...
		if (dogIdentifier == -1) {
			...
			sender.sendMessage(
					ChatColor.AQUA + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.AQUA + mode
							+ ", my dogs!");
		} else {
			...
			if (dogs.get(0) != null) {
				sender.sendMessage(
						ChatColor.AQUA + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.AQUA + mode
								+ ", " + dogs.get(0).getDogColor() + dogs.get(0).getDogName() + ChatColor.RESET + ChatColor.AQUA + "!");
			}
		}
	...
	private boolean commandDogStats(CommandSender sender, int dogIdentifier) {
		...
		if (plugin.useLevels) {
			...
			if (maxExp != 0) {
				...
				experienceString =
						calculatePercentString(percent) + ChatColor.AQUA + "" + ChatColor.BOLD + " [" + ChatColor.DARK_AQUA + df.format(exp)
								+ ChatColor.AQUA + "" + ChatColor.BOLD + "/" + ChatColor.RESET + ChatColor.AQUA + df.format(maxExp)
		...
		if (wolf != null) {
			...
			sender.sendMessage(
					ChatColor.AQUA + "Health: " + healthString + ChatColor.AQUA + "" + ChatColor.BOLD + " [" + ChatColor.DARK_AQUA
							+ df.format(health) + ChatColor.AQUA + "" + ChatColor.BOLD + "/" + ChatColor.RESET + ChatColor.AQUA + df.format(
							maxHealth) + ChatColor.AQUA + "" + ChatColor.BOLD + "]");

			AttributeInstance attackDamage = wolf.getAttribute(Attribute.GENERIC_ATTACK_DAMAGE);
			sender.sendMessage(
					ChatColor.AQUA + "Damage: " + ChatColor.WHITE + (attackDamage != null ? attackDamage.getValue() : 0) + " HP");
		}

		Location dogLoc = dog.getDogLocation();
		if (dogLoc != null) {
			sender.sendMessage(
					ChatColor.AQUA + "Last Seen at: " + ChatColor.DARK_AQUA + "World: " + ChatColor.WHITE + (dogLoc.getWorld() != null ?
							dogLoc.getWorld().getName() :
							"Unknown World") + ChatColor.DARK_AQUA + " X: " + ChatColor.WHITE + df.format(dogLoc.getX())
							+ ChatColor.DARK_AQUA + " Y: " + ChatColor.WHITE + df.format(dogLoc.getY()) + ChatColor.DARK_AQUA + " Z: "
							+ ChatColor.WHITE + df.format(dogLoc.getZ()));
		}
	...
	private boolean commandReviveDog(Player player, CommandSender sender, int dogIdentifier) {
		...
		if (!plugin.revivalUsingPlayerExp) {
			...
			if (player.getLevel() < revivalPrice) {
				sender.sendMessage(
						ChatColor.RED + "You don't have enough power to resurrect the dog. You need " + ChatColor.GOLD + (revivalPrice
								- player.getLevel()) + ChatColor.RED + " more levels!");
				return false;
			}
		...
		for (Dog dog : dogs) {
			...
			if (dogLocation == null) {
				...
				if (wolf == null) {
					sender.sendMessage(
							ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.RED
									+ "Aww bonkers! Seems like your Dog cannot be found... Sorry about that!");
					return false;
				}
			} else {
				if (dogLocation.getChunk().load(false)) {
					...
					sender.sendMessage(
							ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.RED
									+ "Aww bonkers! Seems like your Dog is at a location that cannot be loaded right now!");
					return false;
				}
				useLocation = true;
			}
			...
			if (wolf == null) {
				sender.sendMessage(
						ChatColor.RED + "" + ChatColor.BOLD + "[" + plugin.getChatPrefix() + "] " + ChatColor.RESET + ChatColor.RED
								+ "Aww bonkers! Seems like your Dog cannot be found...");
			...
			String comehereString = plugin.commandComehereString.replace("{chatPrefix}", plugin.getChatPrefix())
	...
	private boolean commandTrade(CommandSender sender, int dogIdentifier, Player receiver, double price) {
		...
		if (dogManager.handleNewTrade(dog, receiver, price)) {
			...
			receiver.sendMessage(
					ChatColor.GOLD + "You have received a dog trade request from " + ChatColor.AQUA + ((Player) sender).getDisplayName()
							+ ChatColor.GOLD + "!\n" + ChatColor.AQUA + ((Player) sender).getDisplayName() + ChatColor.GOLD
							+ " is offering " + dog.getDogColor() + dog.getDogName() + ChatColor.GOLD + (plugin.useLevels ?
							(" (" + ChatColor.AQUA + "Level " + dog.getLevel() + ChatColor.GOLD + ")") :
							"") + " for " + ChatColor.AQUA + (price > 0.0D ? (df.format(price) + "$") : "free") + ChatColor.GOLD + "!\n\n"
							+ ChatColor.GOLD + "Accept the trade request with " + ChatColor.AQUA + "/md tradeaccept\n" + ChatColor.GOLD
							+ "Decline the trade request with " + ChatColor.AQUA + "/md tradedecline\n" + ChatColor.GOLD
